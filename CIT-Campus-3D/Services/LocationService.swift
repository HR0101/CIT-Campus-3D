...
final class LocationService: NSObject {
  ...
  /// 大学のWiFi（SSID一致）に接続中か．
  /// 「学内WiFiに繋がっている＝校内（その授業の棟の中）にいる」とみなす判定に使う．
  /// WiFiの電波は建物内・周辺の数十m程度しか届かないため，校内にいる強い根拠になる．
  /// SSID取得には Access WiFi Information 権限が必要なため，権限の無い環境・Simulatorでは常にfalse．
  var isConnectedToCampusWiFi: Bool {
    guard let ssid = currentSSID, !ssid.isEmpty else { return false }
    return Self.universitySSIDs.contains(ssid)
  }
