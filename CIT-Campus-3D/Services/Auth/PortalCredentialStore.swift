...
final class PortalCredentialStore {
  ...
  /// Keychainストア
  private let keychain = KeychainStore(service: "com.HR.CIT-Campus-3D.portal")
  /// 各項目のアカウントキー
  ...
  init() {
    ...
    hasTOTPSecret = ((try? keychain.readString(account: Account.totpSecret)) ?? nil) != nil
  ...
  func save(_ credentials: PortalCredentials) throws {
    ...
    try keychain.writeString(userID, account: Account.userID)
    try keychain.writeString(password, account: Account.password)

    // シークレットは任意: 入力があれば保存，無ければ既存を削除する
    if secret.isEmpty {
      try keychain.delete(account: Account.totpSecret)
      hasTOTPSecret = false
    } else {
      try keychain.writeString(secret, account: Account.totpSecret)
      hasTOTPSecret = true
    }
  ...
  func loadTOTPSecret() -> String? {
    (try? keychain.readString(account: Account.totpSecret)) ?? nil
  }
