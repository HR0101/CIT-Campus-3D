...
struct ContentView: View {
  ...
  @Environment(AttendanceCredentialStore.self) private var attendanceCredentialStore
  ...
  @AppStorage("lastAttendancePromptedClassStart") private var lastPromptedClassStart: Double = 0
  ...
  var body: some View {
    ...
    .alert(
      "出席しますか？",
      isPresented: Binding(
        get: { attendancePromptLecture != nil },
        set: { if !$0 { attendancePromptLecture = nil } }
      ),
      presenting: attendancePromptLecture
    ) { lectureResult in
      if !lectureResult.lecture.attendanceRoomId.isEmpty,
         let url = URL(string: "https://attendance.is.chibatech.ac.jp/attendance/class_room/\(lectureResult.lecture.attendanceRoomId)") {
        Button("出席する", role: .none) {
          attendanceUrlToOpen = AttendanceDestination(url: url)
        }
      }
      Button("今はしない", role: .cancel) {}
    } message: { lectureResult in
      Text("現在大学のWi-Fiに接続しています。まもなく始まる「\(lectureResult.lecture.subjectName)」の出席を登録しますか？")
    }
    .sheet(item: $attendanceUrlToOpen) { destination in
      AttendanceSheetView(url: destination.url)
    }
    .alert("出席システムへのログインに失敗しました", isPresented: $showAttendanceLoginFailedAlert) {
      Button("OK", role: .cancel) {}
    } message: {
      Text("設定の「出席システム連携」でユーザーID・パスワードを確認してください．")
    }
    .task {
    ...
    .onChange(of: locationService.isConnectedToCampusWiFi) { _, isConnected in
      if isConnected {
        checkAttendancePrompt()
      }
    }
  ...
  private func checkAttendancePrompt() {
    guard locationService.isConnectedToCampusWiFi else { return }
    let now = Date()
    let resolved = resolver.resolveUpcoming(from: lectures, now: now)
    let upcoming = resolver.removingCancellations(resolved, changes: classChanges)

    guard let next = upcoming.first, next.isToday, !next.lecture.attendanceRoomId.isEmpty else { return }

    let timeUntilStart = next.startDate.timeIntervalSince(now)
    // 授業開始前30分〜授業中（開始後）の間であれば出席可能とみなす
    if (timeUntilStart >= 0 && timeUntilStart <= 30 * 60) || next.isOngoing {
      let classStartTimestamp = next.startDate.timeIntervalSince1970
      if lastPromptedClassStart != classStartTimestamp {

        // 認証情報が保存されていなければチェックも自動出席もできないので終了
        // （「ポータルと同じ」設定ならポータルの，そうでなければ出席システム専用に登録した認証情報を使う）
        guard
          let userID = attendanceCredentialStore.effectiveUserID(portalStore: credentialStore),
          let password = attendanceCredentialStore.effectivePassword(portalStore: credentialStore)
        else { return }

        Task {
          let checker = AttendanceChecker()
          let result = await checker.checkStatus(roomID: next.lecture.attendanceRoomId, userID: userID, password: password)

          await MainActor.run {
            switch result {
            case .needsAttendance:
              lastPromptedClassStart = classStartTimestamp
              attendancePromptLecture = next
            case .alreadyAttendedOrNotAvailable:
              // すでに出席済み、または何らかの理由でボタンが出ない場合は、
              // 以降毎回チェックして通信を無駄にしないよう、プロンプト済み扱いにする
              lastPromptedClassStart = classStartTimestamp
            case .loginFailed:
              // 登録済みのID・パスワードで出席システムにログインできなかった場合は，
              // この授業では再試行せず，設定の見直しを促す（サイレントに失敗し続けないようにする）
              lastPromptedClassStart = classStartTimestamp
              showAttendanceLoginFailedAlert = true
            case .networkError:
              // 通信エラーは一時的な可能性があるため，プロンプト済みにはせず次回再試行する
              break
            }
          }
        }
  ...
  private func rescheduleNotifications(upcoming: [NextLectureResult]) async {
    ...
    if settings.enableDepartureReminder,
      ...
      // 「大学周辺でのみ経路を表示」がオンの場合、通知のための経路探索も大学周辺に限る
      let isWithinCampus = !settings.restrictRouteToCampus
        || destination.campus.isWithinRouteVicinity(of: location.coordinate)
