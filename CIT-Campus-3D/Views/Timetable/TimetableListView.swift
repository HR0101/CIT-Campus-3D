...
struct TimetableListView: View {
  ...
  /// nilは学年暦に従う自動表示．手動で別学期も確認できる．
  @State private var semesterOverride: Semester?
  @State private var referenceDate = Date()

  private var automaticSemester: Semester { .current(on: referenceDate) }
  private var selectedSemester: Semester { semesterOverride ?? automaticSemester }
  ...
  private var semesterPicker: some View {
    Picker("学期", selection: $semesterOverride) {
      Text("自動（\(automaticSemester.displayName)）").tag(nil as Semester?)
      ForEach(Semester.allCases) { semester in
        Text(semester.displayName).tag(Optional(semester))
      }
    }
  ...
  private func saveImportedDrafts(_ drafts: [LectureDraft], replaceExisting: Bool) {
    ...
    do {
      ...
      // 前後期の一括登録後も，今日の学年暦に合う学期を表示する．
      referenceDate = Date()
      semesterOverride = nil
    } catch {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 4) {
      HStack(spacing: 8) {
        ...
        Spacer()
        if !lecture.attendanceRoomId.isEmpty,
           let url = URL(string: "https://attendance.is.chibatech.ac.jp/attendance/class_room/\(lecture.attendanceRoomId)") {
          Button {
            onAttendanceTap?(url)
          } label: {
            Text("出席")
...
struct TimetableGrid: View {
  ...
  var body: some View {
    ...
    .confirmationDialog(
      ...
      isPresented: Binding(
        get: { selectedLecture != nil },
        set: { if !$0 { selectedLecture = nil } }
      ),
      titleVisibility: .visible,
      presenting: selectedLecture
    ) { lecture in
      if !lecture.attendanceRoomId.isEmpty,
         let url = URL(string: "https://attendance.is.chibatech.ac.jp/attendance/class_room/\(lecture.attendanceRoomId)") {
        Button("出席を登録") {
      ...
      let activeAssigns = AssignmentLectureMatcher.activeAssignments(for: lecture, in: assignments)
      if activeAssigns.isEmpty {
        Text(lecture.placeText)
      } else {
        let titles = activeAssigns.map { "・\($0.title)" }.joined(separator: "\n")
        Text("\(lecture.placeText)\n\n【未提出の課題】\n\(titles)")
      }
    }
  }
