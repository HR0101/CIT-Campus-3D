    ...
    NavigationStack {
      Form {
        ...
        Section {
          ...
          Text("ユーザーID・パスワード・ワンタイムパスワードのキーを登録すると，ポータルから取り込む際のログイン情報を自動入力します．出席システムのID・パスワードが同じ場合は，この情報がそのまま出席の自動チェックにも使われます（異なる場合は下の「出席システム連携」で個別に登録できます）．認証情報はこの端末内にのみ保存します．")
        }
