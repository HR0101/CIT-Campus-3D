...
struct AcademicCalendar {
  ...
  /// 週間時間割の表示学期．休業中は次の学期，年度末は最後の学期を表示する．
  /// 対象外の年度にはnilを返し，呼び出し元で簡易判定にフォールバックする．
  func displaySemester(on date: Date, calendar: Calendar = .current) -> Semester? {
    guard covers(date, calendar: calendar) else { return nil }
    if let term = term(on: date, calendar: calendar) { return term.semester }
    let key = dayKey(for: date, calendar: calendar)
    return terms.filter { $0.startKey > key }.min { $0.startKey < $1.startKey }?.semester
      ?? terms.max { $0.endKey < $1.endKey }?.semester
  }
