...
時間割画面の「スクショ」から、写真または画像ファイルを選択できます。
ポータルの曜日見出しと1〜10限の表全体が写る、罫線のあるスクショに対応します。
文字認識は端末内で行い、画像を外部サービスへ送信しません。
読み取り後は学期と各授業の科目名・教員・曜日・時限・教室を確認・修正してから登録します。
読み取り漏れは確認画面で追加できます。集中講義は自動取り込みの対象外です。
初期設定は追加登録です。「この学期の時間割を置き換える」をオンにした場合も、他の学期は保持します。
